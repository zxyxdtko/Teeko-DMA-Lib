...
DMAOperationResult DMA::Initialize(const DMAInitializationOptions& options)
{
    ...
    const bool refreshAutomaticMemoryMap =
        options.useMemoryMap && options.memoryMapPath.empty();
    ...
    if (options.useMemoryMap && !memoryMapPath.empty() && memoryMapPath != "auto") {
        ...
        memoryMapAvailable = !refreshAutomaticMemoryMap &&
            std::filesystem::exists(memoryMapPath, error) && !error;
    }
    std::string memoryMapPreparationError;
    if (options.useMemoryMap && !memoryMapAvailable &&
        !memoryMapPath.empty() && memoryMapPath != "auto") {
        const std::string probeDevice = options.device.rfind("fpga", 0) == 0
            ? "fpga" : options.device;
        std::vector<std::string> probeArguments = {
            "", "-device", probeDevice, "-waitinitialize", "-norefresh"
        };
        probeArguments.insert(probeArguments.end(), options.extraArguments.begin(),
            options.extraArguments.end());

        const auto probe = backend->Initialize(probeArguments);
        if (!probe) {
            memoryMapPreparationError = probe.message.empty()
                ? "temporary physical-memory-map session failed"
                : probe.message;
        }
        else {
            std::vector<DMAPhysicalMemoryRange> ranges;
            const auto mapped = backend->GetPhysicalMemoryMap(ranges);
            if (!mapped || ranges.empty()) {
                memoryMapPreparationError = mapped.message.empty()
                    ? "VMMDLL returned an empty physical memory map"
                    : mapped.message;
            }
            else {
                std::ofstream output(memoryMapPath, std::ios::out | std::ios::trunc);
                if (!output) {
                    memoryMapPreparationError =
                    ...
                    if (!output) {
                        memoryMapPreparationError =
                            "writing the physical-memory-map file failed";
                    }
                    else {
                        memoryMapAvailable = true;
                    }
                }
            }
        }
    ...
    if (options.useMemoryMap && !memoryMapAvailable &&
        ...
        return DMAOperationResult::Failure(DMAStatus::NotFound,
            memoryMapPreparationError.empty()
                ? "The requested memory-map file is unavailable: " + memoryMapPath
                : "Unable to prepare the requested memory map: " +
                    memoryMapPreparationError);
    }
    ...
    if (!options.useMemoryMap || options.fallbackWithoutMemoryMap) {
        auto initialized = attemptInitialize(false);
        if (!initialized && !memoryMapPreparationError.empty()) {
            if (!initialized.message.empty())
                initialized.message += "; ";
            initialized.message += "memory-map preparation: " +
                memoryMapPreparationError;
        }
        return initialized;
    }
...
std::vector<uint8_t> DMA::DumpMemoryEx(DWORD pid, uint64_t address, size_t size,
    ...
    const ULONG64 resolvedFlags = DMAEnforceNoCache(flags);
    ...
    while (offset < size) {
        ...
        const auto result = backend->ReadMemory(pid, address + offset,
            buffer.data() + offset, chunkSize, resolvedFlags, bytesRead);
        if (!result && result.status != DMAStatus::PartialTransfer) {
            buffer.clear();
            return buffer;
        }
        if (bytesRead != chunkSize &&
            !(resolvedFlags & VMMDLL_FLAG_ZEROPAD_ON_FAIL)) {
