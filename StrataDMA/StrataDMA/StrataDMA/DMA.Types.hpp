...
// StrataDMA intentionally bypasses VMMDLL's target-memory data cache for every
// normal and scatter read. Remove flags that explicitly request cached data so
// callers cannot accidentally defeat the library-wide live-memory policy.
inline constexpr ULONG64 DMAEnforceNoCache(ULONG64 flags) noexcept
{
    return (flags & ~(static_cast<ULONG64>(VMMDLL_FLAG_FORCECACHE_READ) |
        static_cast<ULONG64>(VMMDLL_FLAG_CACHE_RECENT_ONLY))) |
        static_cast<ULONG64>(VMMDLL_FLAG_NOCACHE);
}
