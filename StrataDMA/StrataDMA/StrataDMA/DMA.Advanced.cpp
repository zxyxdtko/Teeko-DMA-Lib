...
{
    return DMAScatterBatch(backend, pid == 0 ? targetPID : pid,
        static_cast<DWORD>(DMAEnforceNoCache(flags)));
}
    ...
    size_t size, ULONG64 flags) const
{
    ...
    result.value.flags = resolvedFlags;
    ...
    result.operation = backend->ReadMemory(targetPID, address,
        result.value.bytes.data(), static_cast<DWORD>(size), resolvedFlags,
        transferred);
    ...
    result.operation.flags = resolvedFlags;
    ...
    uint32_t relativeAddress, void* buffer, size_t size, ULONG64 flags) const
{
    ...
    auto result = backend->ReadRegistryHive(cmHiveAddress, relativeAddress,
        buffer, static_cast<DWORD>(size), resolvedFlags, transferred);
    result.requestedBytes = size;
    result.transferredBytes = transferred;
    result.flags = resolvedFlags;
    return result;
