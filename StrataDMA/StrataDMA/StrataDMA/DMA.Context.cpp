...
DMAScatterBatch::DMAScatterBatch(std::shared_ptr<IVmmBackend> backend,
    ...
    : backend_(std::move(backend)), pid_(pid),
      flags_(static_cast<DWORD>(DMAEnforceNoCache(flags)))
{
...
DMAMemoryContext::DMAMemoryContext(std::shared_ptr<IVmmBackend> backend,
    ...
    : backend_(std::move(backend)), pid_(pid),
      defaultFlags_(DMAEnforceNoCache(defaultFlags))
{
}
...
{
    const ULONG64 resolvedFlags = DMAEnforceNoCache(
        flags == 0 ? defaultFlags_ : flags);
...
{
    return DMAScatterBatch(backend_, pid_,
    ...
    if (buffer && size != 0 &&
        ...
        for (const auto& block : cache_) {
            ...
            if (address >= block.address && end <= blockEnd) {
                ...
                result.address = address;
                result.pid = pid_;
                result.flags = DMAEnforceNoCache(
                    flags == 0 ? defaultFlags_ : flags);
                return result;
            }
        }
