...
STRATA_TEST_CASE(initialization_generates_a_missing_physical_memory_map)
{
    MockDmaFixture fixture;
    const auto mapPath = std::filesystem::temp_directory_path() /
        "strata_dma_generated_map_test.txt";
    std::error_code error;
    std::filesystem::remove(mapPath, error);

    DMAInitializationOptions options;
    options.device = "mock://device";
    options.useMemoryMap = true;
    options.memoryMapPath = mapPath.string();
    options.fallbackWithoutMemoryMap = false;

    const auto initialized = fixture.dma.Initialize(options);
    STRATA_REQUIRE(initialized);
    STRATA_REQUIRE(fixture.backend->initializeCalls.load() == 2);
    STRATA_REQUIRE(std::filesystem::exists(mapPath));
    STRATA_REQUIRE(!fixture.backend->initializeArguments.empty());
    STRATA_REQUIRE(fixture.backend->initializeArguments.front().empty());
    STRATA_REQUIRE(HasArgument(fixture.backend->initializeArguments, "-memmap"));
    STRATA_REQUIRE(HasArgument(fixture.backend->initializeArguments,
...
STRATA_TEST_CASE(structured_reads_and_writes_report_complete_context)
{
    ...
    auto forcedUncached = fixture.dma.Read<uint32_t>(address,
        VMMDLL_FLAG_FORCECACHE_READ | VMMDLL_FLAG_CACHE_RECENT_ONLY |
        VMMDLL_FLAG_NOPAGING);
    const ULONG64 expectedUncachedFlags =
        VMMDLL_FLAG_NOCACHE | VMMDLL_FLAG_NOPAGING;
    STRATA_REQUIRE(forcedUncached && forcedUncached.value == expected);
    STRATA_REQUIRE(forcedUncached.operation.flags == expectedUncachedFlags);
    STRATA_REQUIRE(fixture.backend->lastReadFlags == expectedUncachedFlags);
...
STRATA_TEST_CASE(memory_contexts_bind_pid_flags_and_translation)
{
    ...
    STRATA_REQUIRE(fixture.backend->lastReadFlags ==
        (VMMDLL_FLAG_NOCACHE | VMMDLL_FLAG_NOPAGING));
